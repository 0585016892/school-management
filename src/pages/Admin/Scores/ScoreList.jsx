...
import useHasRole from "../../../hooks/useHasRole"; // Import hook kiểm tra role
import { ROLES } from "../../../constants/roles"; // Import bộ Enum Role

const { Title, Text } = Typography;

const ScorePage = () => {
  const { hasRole } = useHasRole(); // Kiểm tra role hiện tại của user

  // Kiểm tra xem user hiện tại có quyền chỉnh sửa điểm số hay không
  const canEditScore = hasRole(
    ROLES.ADMIN,
    ROLES.PRINCIPAL,
  ...
  const updateCell = (id, field, value) => {
    if (!canEditScore) return; // Bảo vệ logic ghi
    setStudents((prev) =>
    ...
    },
    {
      ...
      render: (_, r) => (
        <InputNumber
          ...
          max={10}
          step={0.25}
          disabled={!canEditScore} // Vô hiệu hóa nếu không có quyền sửa điểm
          controls={false}
          variant="filled"
    ...
    <div style={styles.container}>
      ...
      >
        <Row gutter={[16, 16]} justify="space-between" align="middle">
          ...
          {/* NÚT HÀNH ĐỘNG GHI NHẬN: Bọc CanRole để ẩn với Student/Parent */}
